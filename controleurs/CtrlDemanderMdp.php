...
if ( ! isset ($_POST ["txtPseudo"]) || ! isset ($_POST ["txtMail"])== true) {
    ...
    if ($pseudo == '' || $mail == '') {
		...
		if ( ! $dao->existePseudoUtilisateur($pseudo) )  {
			...
			$message = "Erreur : pseudo ou adresse mail inexistant.";
			$typeMessage = 'avertissement';
			$themeFooter = $themeProbleme;
			...
			$mailPseudo = $unUtilisateur->getAdrMAil();
			if ( $mail === $mailPseudo) {
				...
				// si le login (pseudo ou adresse mail) n'existe pas, retour à la vue
				$message = "Erreur : le pseudo ne correspond pas à l'adresse mail.";
				$typeMessage = 'avertissement';
				$themeFooter = $themeProbleme;
				unset($dao);		// fermeture de la connexion à MySQL
				include_once ('vues/VueDemanderMdp.php');
